      ...
      for (let y = 0; y < Math.floor(h / 2); y++) {
        ...
        let ceilY = posY + rowDistance * (dirY - planeY);
          ...
          if (tex && tex.width > 0 && tex.height > 0) {
            ...
            const usableWidthC = tex.width - (TEXTURE_BORDER_CLIP * 2);
            const usableHeightC = tex.height - (TEXTURE_BORDER_CLIP * 2);
            const txC = TEXTURE_BORDER_CLIP + (Math.floor(Math.abs(ceilX - tileX) * usableWidthC) % usableWidthC);
            const tyC = TEXTURE_BORDER_CLIP + (Math.floor(Math.abs(ceilY - tileY) * usableHeightC) % usableHeightC);
            // Maximum quality texture sampling
            ctx.drawImage(tex, txC, tyC, textureSampleSize, textureSampleSize, x, y, pxStep, 1);
          }
    ...
    if (floorTex) {
      for (let y = Math.floor(h / 2) + 1; y < h; y++) {
        ...
        for (let x = 0; x < w; x += pxStep) {
          ...
          if (tex && tex.width > 0 && tex.height > 0) {
            // Pre-compute constants for maximum speed
            const usableWidthF = tex.width - (TEXTURE_BORDER_CLIP * 2);
            const usableHeightF = tex.height - (TEXTURE_BORDER_CLIP * 2);
            const txF = TEXTURE_BORDER_CLIP + (Math.floor(Math.abs(floorX - tileX) * usableWidthF) % usableWidthF);
            const tyF = TEXTURE_BORDER_CLIP + (Math.floor(Math.abs(floorY - tileY) * usableHeightF) % usableHeightF);
            // Maximum quality texture sampling
            ctx.drawImage(tex, txF, tyF, textureSampleSize, textureSampleSize, x, y, pxStep, 1);
          }
       ...
       if (allWallTextures.length > 0) {
         ...
         const baseboardTop = drawEnd - baseboardHeight;
          ...
          if (texturesRef.current.floor) {
            ...
            const usableFloorWidth = floorTex.width - (TEXTURE_BORDER_CLIP * 2);
            const usableFloorHeight = floorTex.height - (TEXTURE_BORDER_CLIP * 2);
            const texX = TEXTURE_BORDER_CLIP + (Math.floor((wallX * 64) % usableFloorWidth));
            
            ctx.globalAlpha = side === 1 ? 0.8 : 1.0; // Slightly darker on side walls
            // Larger sample (8px) for smoother edges
            ctx.drawImage(
              floorTex,
              texX, TEXTURE_BORDER_CLIP, 8, usableFloorHeight,
