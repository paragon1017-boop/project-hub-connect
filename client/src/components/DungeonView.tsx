...
export function DungeonView({ gameData, className }: DungeonViewProps) {
  ...
  const draw = () => {
    ...
    // Stone boulder fascia at floor/wall transition (bottom of view)
    const floorFasciaY = h - 10;
    const floorFasciaHeight = 12;
    
    // Seeded random for consistent boulder pattern
    let floorFasciaSeed = 54321;
    const floorFasciaRandom = () => {
      floorFasciaSeed = (floorFasciaSeed * 1103515245 + 12345) & 0x7fffffff;
      return floorFasciaSeed / 0x7fffffff;
    };
    
    // Draw stone boulders across the floor fascia
    let floorBoulderX = 0;
    while (floorBoulderX < w) {
      const boulderW = 14 + Math.floor(floorFasciaRandom() * 20);
      const boulderH = 9 + Math.floor(floorFasciaRandom() * 4);
      const boulderYOffset = Math.floor(floorFasciaRandom() * 2);
      
      // Boulder base color with variation (slightly darker than ceiling)
      const baseGray = 55 + Math.floor(floorFasciaRandom() * 25);
      const rVar = Math.floor(floorFasciaRandom() * 12) - 6;
      const gVar = Math.floor(floorFasciaRandom() * 10) - 5;
      const bVar = Math.floor(floorFasciaRandom() * 8) - 4;
      
      const r = baseGray + rVar;
      const g = baseGray + gVar - 2;
      const b = baseGray + bVar - 3;
      
      // Main boulder body
      ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
      ctx.beginPath();
      ctx.moveTo(floorBoulderX + 2, floorFasciaY + boulderYOffset);
      ctx.lineTo(floorBoulderX + boulderW - 2, floorFasciaY + boulderYOffset);
      ctx.lineTo(floorBoulderX + boulderW, floorFasciaY + boulderYOffset + 3);
      ctx.lineTo(floorBoulderX + boulderW - 1, floorFasciaY + boulderYOffset + boulderH - 2);
      ctx.lineTo(floorBoulderX + boulderW - 3, floorFasciaY + boulderYOffset + boulderH);
      ctx.lineTo(floorBoulderX + 3, floorFasciaY + boulderYOffset + boulderH);
      ctx.lineTo(floorBoulderX, floorFasciaY + boulderYOffset + boulderH - 3);
      ctx.lineTo(floorBoulderX + 1, floorFasciaY + boulderYOffset + 2);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(floorBoulderX + 3, floorFasciaY + boulderYOffset + 1);
      ctx.lineTo(floorBoulderX + boulderW - 3, floorFasciaY + boulderYOffset + 1);
      ctx.lineTo(floorBoulderX + boulderW - 4, floorFasciaY + boulderYOffset + 3);
      ctx.lineTo(floorBoulderX + 4, floorFasciaY + boulderYOffset + 3);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(floorBoulderX + 4, floorFasciaY + boulderYOffset + boulderH - 1);
      ctx.lineTo(floorBoulderX + boulderW - 4, floorFasciaY + boulderYOffset + boulderH - 1);
      ctx.lineTo(floorBoulderX + boulderW - 2, floorFasciaY + boulderYOffset + boulderH - 3);
      ctx.lineTo(floorBoulderX + 2, floorFasciaY + boulderYOffset + boulderH - 3);
      ctx.closePath();
      ctx.fill();
      
      // Cracks and texture on boulder
      if (floorFasciaRandom() > 0.5) {
        ctx.strokeStyle = `rgba(25, 22, 18, 0.5)`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        const crackStartX = floorBoulderX + 4 + Math.floor(floorFasciaRandom() * (boulderW - 8));
        ctx.moveTo(crackStartX, floorFasciaY + boulderYOffset + 2);
        ctx.lineTo(crackStartX + (floorFasciaRandom() - 0.5) * 5, floorFasciaY + boulderYOffset + boulderH - 2);
        ctx.stroke();
      }
      
      // Mortar/gap between boulders
      ctx.fillStyle = 'rgba(15, 12, 10, 0.7)';
      ctx.fillRect(floorBoulderX + boulderW - 1, floorFasciaY - 1, 2, floorFasciaHeight + 2);
      
      floorBoulderX += boulderW + 1;
    }
    
    // Shadow above floor fascia (transition from wall)
    ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
    ctx.fillRect(0, floorFasciaY - 2, w, 3);
    
    // Dark line at very bottom
    ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
